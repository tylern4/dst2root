...
int MonteCarlo(std::string file = "test.root", double BEAM = 2.2) {
  ...
  clas12->SetBranchAddress("lund_particle", &Lund_particle);
  ...
  for (int current_event = 0; current_event < num_of_events; current_event++) {
    ...
    if (pid->at(0) == 11) {
      ...
      deltaP[3]->Fill((p->at(0) - Lund_particle->at(0).P()) / Lund_particle->at(0).P());
    }
    ...
    for (size_t mc_part = 1; mc_part < MC_pid->size(); mc_part++) {
      for (size_t part = 1; part < pid->size(); part++) {
        if (MC_pid->at(mc_part) == pid->at(part)) {
          ...
          deltaP[7]->Fill((p->at(part) - Lund_particle->at(mc_part).P()) / Lund_particle->at(mc_part).P());
        }
      }
    }
  }
